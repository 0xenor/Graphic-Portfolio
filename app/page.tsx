    ...
    const o2 = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        ...
        skills.forEach((s, i) => {
          const duration = 1400;
          const steps = 60;
          const increment = s.pct / steps;
          let current = 0;
          const timer = setInterval(() => {
            current += increment;
            if (current >= s.pct) { current = s.pct; clearInterval(timer); }
            setCounters(prev => { const next = [...prev]; next[i] = Math.round(current); return next; });
          }, duration / steps);
        });
      }
    }, { threshold: 0.1 });
  ...
  return (
    <main className="relative w-full min-h-screen bg-black flex flex-col items-center overflow-x-hidden">
      <style>{`
        ...
        .btn-primary  { font-family:'Syne',sans-serif; background:#bb00ff; border:1.5px solid #bb00ff; color:white; padding:14px 32px; border-radius:8px; font-weight:900; font-size:13px; letter-spacing:0.2em; text-transform:uppercase; text-decoration:none; transition:background 0.3s,color 0.3s; white-space:nowrap; }
        .btn-primary:hover  { background:transparent; color:#bb00ff; }
        .btn-secondary { font-family:'Syne',sans-serif; background:transparent; border:1.5px solid rgba(187,0,255,0.5); color:#bb00ff; padding:14px 36px; border-radius:8px; font-weight:900; font-size:13px; letter-spacing:0.2em; text-transform:uppercase; text-decoration:none; transition:background 0.3s,color 0.3s,border-color 0.3s; white-space:nowrap; }
        ...
        /* ── Skills ── */
        .sk-title { font-family:'Syne',sans-serif; font-weight:900; font-size:clamp(3rem,10vw,7rem); letter-spacing:-0.04em; text-transform:uppercase; color:white; line-height:1; margin-bottom:72px; text-align:center; }
        ...
        .sk-name { font-family:'Syne',sans-serif; font-size:10px; font-weight:600; color:rgba(255,255,255,0.3); letter-spacing:0.06em; text-transform:uppercase; margin-top:10px; text-align:center; }

        .proj-title { font-family:'Syne',sans-serif; font-weight:900; font-size:clamp(2rem,8vw,7rem); letter-spacing:-0.04em; text-transform:uppercase; color:white; line-height:1; margin-bottom:48px; text-align:center; width:100%; overflow:hidden; }
        ...
        .proj-name { font-family:'Syne',sans-serif; font-size:15px; font-weight:800; color:white; letter-spacing:-0.02em; }
        .proj-badge {
        ...
        .proj-add-txt  { font-family:'Syne',sans-serif; font-size:11px; font-weight:600; letter-spacing:0.1em; text-transform:uppercase; color:rgba(255,255,255,0.2); margin-top:8px; }
      ...
      <section id="home" className="relative z-10 flex flex-col items-center justify-center text-center w-full min-h-screen px-6">
        <div className="w-full max-w-[900px] flex flex-col items-center gap-0">
          <SplitTextComponent text="Hello!"          className="syne text-[clamp(3.5rem,12vw,7rem)] font-[900] text-white" delay={250} />
          <SplitTextComponent text="I'm Fahed Hadji" className="syne text-[clamp(2.2rem,9vw,6rem)]  font-[900] text-white" delay={200} />
          <div className="flex items-center justify-center mt-10 mb-12">
            <ShinyText text="Graphic Designer" speed={5} className="syne text-[clamp(1.2rem,4vw,2.25rem)] font-extrabold" />
          </div>
      ...
      <section id="about" ref={aboutRef} className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center py-24 overflow-hidden">
        ...
        <div className={`about-grid fade-up ${isVisible ? "visible" : "hidden"}`}>
          <div className="img-col">
            <img src="pro.png" alt="Fahed Hadji" className="profile-img" style={{ marginBottom:"60px" }} />
          </div>
          <div className="text-col">
            <div className="accent-line" />
            <h3 className="syne" style={{ margin:0, fontWeight:900, textTransform:"uppercase", letterSpacing:"-0.04em", lineHeight:1.1, fontSize:"clamp(1.6rem,4vw,6rem)", whiteSpace:"nowrap" }}>
            ...
            <div style={{ maxWidth:500 }}>
              <ShinyText
                ...
                disabled={false} speed={3} className="syne text-white/70 text-base md:text-lg font-medium leading-relaxed tracking-tight" />
            </div>
            <div style={{ maxWidth:500 }}>
              <ShinyText text="I have collaborated with diverse clients producing social media assets, advertising visuals, thumbnails, and brand identity graphics — always with precision, creativity, and strict respect for deadlines."
                disabled={false} speed={3} className="syne text-white/50 text-sm md:text-base font-medium leading-relaxed tracking-tight" />
            </div>
            <div className="stats-row">
              {[{ num:"5+", label:"Years Exp." },{ num:"10+", label:"Clients" },{ num:"∞", label:"Creativity" }].map(s => (
                <div key={s.label} style={{ display:"flex", flexDirection:"column", gap:4 }}>
                  <span className="syne" style={{ fontSize:"clamp(1.8rem,4vw,2.5rem)", fontWeight:900, color:"#bb00ff", lineHeight:1 }}>{s.num}</span>
                  <span className="syne" style={{ fontSize:11, fontWeight:600, letterSpacing:"0.15em", textTransform:"uppercase", color:"rgba(255,255,255,0.35)" }}>{s.label}</span>
                </div>
              ))}
            </div>
      ...
      <section id="contact" className="contact-section">
        ...
        <div className="contact-top">
          ...
          <a href="mailto:FahdHadji19@gmail.com" className="contact-btn">
            Send a Message 
          </a>
        </div>
        ...
        <div className="contact-bottom">
          <span className="contact-copy" style={{ color: "white" }}>© 2026 Mr Fahed Designer — All rights reserved. <br />Designed & Built by @Ilyass Ag. <br />My Instagram : ilyass._ag</span>
          <div className="socials">
            <a href="https://www.instagram.com/mr_fahed_designer/" target="_blank" rel="noreferrer" className="social-link" aria-label="Instagram">
            ...
            <a href="https://www.tiktok.com/@fahddesigner19?lang=fr" target="_blank" rel="noreferrer" className="social-link" aria-label="TikTok">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.32 6.32 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.78 1.52V6.75a4.85 4.85 0 01-1.01-.06z"/>
              </svg>
            </a>

            <a href="https://wa.me/212644300095" target="_blank" rel="noreferrer" className="social-link" aria-label="WhatsApp">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
