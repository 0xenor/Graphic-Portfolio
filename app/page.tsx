...
function DisclaimerTyping({ triggerKey }: { triggerKey: number }) {
  ...
  useEffect(() => {
    ...
    if (el) { el.textContent = ''; el.classList.remove('typed'); }
    if (intervalRef.current) clearInterval(intervalRef.current);
    let i = 0;

    const startTyping = () => {
      if (el) el.classList.add('typed');
      intervalRef.current = setInterval(() => {
        i++;
        if (el) el.textContent = DISCLAIMER_TEXT.slice(0, i);
  ...
  return (
    <div ref={containerRef} className="disclaimer-wrap">
      <span className="disclaimer-emoji" aria-hidden="true">⚠️</span>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      resizeTimer.current = setTimeout(computeMasonry, 150);
    };
    ...
    <main className="relative w-full min-h-screen flex flex-col items-center overflow-x-hidden" style={{ background: "#0d0d0d" }}>
      ...
      <style>{`
        ...
        *, *::before, *::after {
        ...
        .h-btn-ghost {
          ...
          color:rgba(255,255,255,.50); font-size:14px; font-weight:600;
          letter-spacing:.04em; text-decoration:none;
          border:1px solid rgba(255,255,255,.14); cursor:pointer; min-width:230px;
          transition:background .2s ease,border-color .2s ease,color .2s ease,transform .22s cubic-bezier(.34,1.56,.64,1);
          white-space:nowrap; will-change:transform;
          -webkit-user-select:none; user-select:none;
        }
        .h-btn-ghost:hover  { background:rgba(255,255,255,.07); border-color:rgba(255,255,255,.32); color:#fff; transform:translateY(-3px); }
        ...
        .h-arrow {
          ...
          animation:hArrow 2.5s ease-in-out infinite;
        ...
        .profile-img {
          ...
          display:block; object-fit:contain; user-select:none; margin-top:-40px;
        }
        ...
        .tab {
          ...
          border:1.5px solid rgba(255,255,255,.1); color:rgba(255,255,255,.45);
        ...
        .disclaimer-wrap {
          ...
          border:1px solid rgba(255,200,0,.12); border-radius:12px;
          background:rgba(255,200,0,.04); max-width:860px; width:100%;
          position:relative; z-index:1;
        }
        .disclaimer-emoji { font-size:17px; flex-shrink:0; animation:pulse-warn 2.2s ease-in-out infinite; display:inline-block; }
        .disclaimer-text {
          font-size:13px; font-weight:500; color:rgba(255,220,80,.65);
          line-height:1.7; text-align:center; direction:rtl;
          opacity:0; transition:opacity .4s ease; min-height:1.5em;
        }
        ...
        .contact-btn {
          ...
          text-decoration:none; padding:18px 48px; border-radius:999px;
          background:#bb00ff; color:#fff; border:1.5px solid #bb00ff;
          transition:background .25s ease,color .25s ease,transform .22s cubic-bezier(.34,1.56,.64,1);
        ...
        .social-icon {
          ...
          transition:transform .22s cubic-bezier(.34,1.56,.64,1),background .2s ease,color .2s ease,border-color .2s ease,box-shadow .2s ease;
          will-change:transform;
        }
      ...
      <section id="home" className="h-section">
        ...
        <div className="h-inner">
          ...
          <h1 className="h-headline">I'm Fahed<br />Hadji</h1>
          <p className="h-body">
          ...
          <div className="h-cta">
            <a href="#projects" className="h-btn">
              View My Work
              <span className="h-btn-icon" aria-hidden="true">
          ...
          <div className={`text-col about-animate${aboutVisible ? " in" : ""}`} style={{ transitionDelay: "0s" }}>
            ...
            <div className="about-text-wrap" style={{ maxWidth: 700, paddingLeft: 10 }}>
              <ShinyText
                ...
                className="text-white/70 text-lg md:text-xl font-bold leading-relaxed"
              />
            </div>
      ...
      <section
        id="skills"
        ref={skillsRef}
        className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center overflow-hidden"
        style={{ padding: "80px 40px", background: "transparent" }}
      >
        <div className="orb" aria-hidden="true"
          style={{ width: 600, height: 400, background: "rgba(187,0,255,0.07)", filter: "blur(130px)", bottom: -100, left: "50%", transform: "translateX(-50%) translateZ(0)" }} />

        <h2 className="sk-title">My <span style={{ color: "#bb00ff" }}>SKILLS</span></h2>

        <div className="sk-grid">
          {skills.map((s, i) => (
            <div key={s.name} className={`sk-card${skillsVisible ? " show" : ""}`} style={{ transitionDelay: `${i * 100}ms` }}>
      ...
      <section
        id="projects"
        ref={projRef}
        className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center overflow-x-hidden"
        style={{ padding: "80px 12px", background: "transparent" }}
      >
        <div className="orb" aria-hidden="true"
          style={{ width: 500, height: 500, background: "rgba(187,0,255,0.06)", filter: "blur(130px)", top: -100, right: -100 }} />

        <h2 className={`proj-title reveal${projVisible ? " in" : ""}`} style={{ transitionDelay: "0ms" }}>
          My <span style={{ color: "#bb00ff" }}>PROJECTS</span>
        </h2>

        <div className={`tabs reveal${projVisible ? " in" : ""}`} style={{ transitionDelay: "80ms" }}>
          {CATS.map(c => (
            <button key={c} className={`tab${activeTab === c ? " active" : ""}`} onClick={() => handleTabClick(c)}>{c}</button>
          ))}
        </div>
        ...
        <div
          ref={masonryRef}
          className={`proj-grid reveal${projVisible ? " in" : ""}`}
          style={{ maxWidth: 1100, margin: "0 auto", height: masonryHeight || undefined }}
        >
          {filtered.map((p, i) => (
            <div
              key={p.id}
            ...
            >
                ...
                : (
                  <div className="proj-placeholder">
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 32, color: "rgba(255,255,255,.15)" }} aria-hidden="true">＋</span>
                      <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(255,255,255,.2)", marginTop: 8 }}>Add Image</span>
                    </div>
                  </div>
                )
              }
              <div className="proj-overlay" aria-hidden="true">
                ...
                <span className="proj-badge" style={{ background: `${catColors[p.category]}22`, color: catColors[p.category] }}>{p.category}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="contact" className="contact-section">
        ...
        <div className="relative z-10 flex flex-col items-center max-w-4xl w-full">
          <span className="mb-6 inline-block px-4 py-1 border border-white/10 rounded-full uppercase text-[#bb00ff]"
            style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.3em" }}>
            Let's Connect
          </span>
          ...
          <a href="https://wa.me/212718982539" target="_blank" rel="noreferrer noopener" className="contact-btn">
        ...
        <div className="contact-bottom">
          <div className="footer-left">
            <span style={{ fontSize: 15, fontWeight: 700, color: "white", letterSpacing: "0.02em" }}>FahdHadji19@gmail.com</span>
          ...
          <div className="footer-center">
            <a href="https://wa.me/212718982539" target="_blank" rel="noreferrer noopener" className="social-icon wa" aria-label="WhatsApp">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
            </a>
            <a href="https://www.instagram.com/mr_fahed_designer/" target="_blank" rel="noreferrer noopener" className="social-icon ig" aria-label="Instagram">
            ...
            <a href="https://www.tiktok.com/@fahddesigner19?lang=fr" target="_blank" rel="noreferrer noopener" className="social-icon tt" aria-label="TikTok">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <div className="footer-right">
            <span style={{ fontSize: 15, fontWeight: 900, color: "rgba(255,255,255,0.22)", letterSpacing: "0.04em" }}>
              Designed &amp; Built by{" "}
              <a href="https://instagram.com/ilyass._ag" target="_blank" rel="noreferrer noopener"
